...
#def handle(args, emotion = ['喜歡', '悲傷', '噁心', '憤怒', '開心', '其它'], text = ""):
        self.device = args.gpu
        self.beam = args.beam
        self.pwd = args.root_path
        self.max_len = args.max_len
        self.gpt2 = GPT2LMHeadModel.from_pretrained(args.pretrained_gpt)
        self.gpt2_tokenizer = BertTokenizer.from_pretrained('bert-base-chinese')
        self.gpt_wrapper = GPT2Wrapper(self.gpt2, tokenizer = self.gpt2_tokenizer, device = self.device)
        self.Q_A = Q(gpt_tokenizer = self.gpt2_tokenizer, bert_name = 'bert-base-chinese', device = self.device)
        self.Q_B = Q(gpt_tokenizer = self.gpt2_tokenizer, bert_name = 'bert-base-chinese', device = self.device)
        self.test_set = GPT2DataSet(tokenizer = self.gpt2_tokenizer, length_upper_bound = 70, length_lower_bound = 60, status = 'test', dataset_root_path = args.dataset_folder, root_path = args.root_path)
        #bleu2, bleu4, accuracy, candidates, targets = bleu_accuracy(test_set, gpt_wrapper, device = device, max_len = args.max_len)
        #perplexity_score = perplexity(test_set, gpt_wrapper, device = device, max_len = 40)
        #print(f"BLEU2: {bleu2} BLEU4: {bleu4} Acc: {accuracy} Perplexity: {perplexity_score} ")
        if os.path.exists(args.model_path) and args.loading_mode != 2:
            ckpt = torch.load(args.model_path)
            if args.loading_mode != 1:
                self.gpt_wrapper.load_state_dict(ckpt['GPT'])
            if args.loading_mode != 3:
                self.Q_A.load_state_dict(ckpt['Q_A'])
                self.Q_B.load_state_dict(ckpt['Q_B'])
    # else:
    #     print("model file doesn't exist")
    def __call__(self, emotion = ['喜歡', '悲傷', '噁心', '憤怒', '開心', '其它'], text = ""):
        input_string = text
        prev_utterance = ""
        final_response = ""
        #while input_string != "quit":
        print("inputed string :", input_string)
        prev_utterance += input_string
        responses, selected_index, prev_utterance, scores = conversation(prev_utterance, self.gpt_wrapper, self.Q_A, self.Q_B, self.gpt2_tokenizer, beam = self.beam, emotion = emotion, device = self.device, max_len = self.max_len)
        print("Candidate Sentences")
        for i in range(self.beam):
            print(self.gpt2_tokenizer.decode(responses[i], skip_special_tokens = True).replace(" ", ""), float(scores[i]))
        print(f"Selected index : {selected_index}\nSentence : {self.gpt2_tokenizer.decode(responses[selected_index], skip_special_tokens = True).replace(' ', '')}")
        final_response = self.gpt2_tokenizer.decode(responses[selected_index], skip_special_tokens = True).replace(' ', '')
        prev_utterance = self.gpt2_tokenizer.decode(prev_utterance, skip_special_tokens = True).replace(" ", "")
            # if prev_utterance[-1] != ']':
            #     prev_utterance += f"[{emotion[random.randint(0, 5)]}]"
            # input_string = input("Enter String To Converse, Enter quit to quit : ")
