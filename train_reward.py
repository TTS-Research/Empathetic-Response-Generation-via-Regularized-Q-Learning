...
def get_args_parser():
    ...
    parser.add_argument('--coherence_weight', default=0)
...
def train_one_epoch(epoch: int, gpt: GPT2Wrapper, Q_A: Q, Q_B: Q,  optimizer: torch.optim.Optimizer = None, R: Reward = None,
                    dataset: Iterable = None, device: torch.device = 'cuda', batch_size=4, max_len=100, beam=3, update_time_per_episode=10, criterion=nn.MSELoss(), kl_control=0.01,
                    gpt_loss_coefficient=0.1, output_dir='./', gpt_update_every_step=100):
    gpt.train()
    ...
    with tqdm(total=len(dataset) // batch_size) as t:
        for step in range(len(dataset) // batch_size):
            ...
            # if step % gpt_update_every_step == 0:
            #     gpt = copy.deepcopy(gpt_prime)
            if True:
                prev_utterance = []
                response = []
                ...
                for update_time in range(update_time_per_episode):
                    ...
                    total_loss = (q_loss + kl_control * kl_loss +
                                  gpt_loss_coefficient * gpt_loss) / gpt_update_every_step
                    optimizer.zero_grad()
                    total_loss.backward()
                    optimizer.step()
                    # if step % gpt_update_every_step == 0:
